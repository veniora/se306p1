...
void vectorCall(Project2Sample::array msg) {
   int i;
   int j = 0;
   for (i = 0; i < 6; i++) {
	...
	ROS_INFO("theta: %f", msg.THETA[i]);
        ++j;
   }
}

void callback(Project2Sample::leader msg) {
   //ROS_INFO("I heard: [%d] is the leader", msg.leaderID);
}
...
int main(int argc, char **argv){
    ros::init(argc, argv, "CaculatePosition");
    ros::NodeHandle n;
    ros::Subscriber vectorCoord = n.subscribe<Project2Sample::array>("Robot_coordinates",1000, vectorCall);
    ros::Subscriber leaderSub = n.subscribe<Project2Sample::leader>("Robot_leader",1000, callback);
    ros::spin();
    return 0;
}
