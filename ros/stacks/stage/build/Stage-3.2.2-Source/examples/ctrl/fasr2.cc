	 ...
	 if( ! ObstacleAvoid() )
		{
		  ...
		  if( closest && (closest_heading_error < 1.0 ) && (closest_range < 2.0) )		 
			 {
				a_error += 0.5 * closest_heading_error;
				//a_error += 0.5 * closest_bearing; // turn towards him

// 				if( closest_range < 2.0 )
// 				  pos->SetXSpeed( cruisespeed * 0.5 );

// 				if( closest_range > 2.0 )
// 				  pos->SetXSpeed( cruisespeed * 1.5 );
			 }
