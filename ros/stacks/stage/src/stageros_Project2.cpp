...
class StageNode
{
  ...
  public:
    ...
    void cmdvelReceived2(int idx, const boost::shared_ptr<geometry_msgs::Twist const>& msg2);
...
void
StageNode::cmdvelReceived2(int idx, const boost::shared_ptr<geometry_msgs::Twist const>& msg)
{
  boost::mutex::scoped_lock lock(msg_lock);
  this->positionmodels[idx]->SetSpeed(msg->linear.x, 
                                      msg->linear.y, 
...
void
StageNode::cmdvelReceived3(int idx, const boost::shared_ptr<geometry_msgs::Twist const>& msg)
{
  boost::mutex::scoped_lock lock(msg_lock);
  this->positionmodels[idx]->SetSpeed(msg->linear.x, 
                                      msg->linear.y, 
...
void
StageNode::cmdvelReceived4(int idx, const boost::shared_ptr<geometry_msgs::Twist const>& msg)
{
  boost::mutex::scoped_lock lock(msg_lock);
  this->positionmodels[idx]->SetSpeed(msg->linear.x, 
                                      msg->linear.y, 
...
void
StageNode::cmdvelReceived5(int idx, const boost::shared_ptr<geometry_msgs::Twist const>& msg)
{
  boost::mutex::scoped_lock lock(msg_lock);
  this->positionmodels[idx]->SetSpeed(msg->linear.x, 
                                      msg->linear.y, 
...
StageNode::SubscribeModels()
{
  ...
  for (size_t r = 0; r < this->positionmodels.size(); r++)
  {
    ...
    if (r == 0 )
	{
		laser_pubs_.push_back(n_.advertise<sensor_msgs::LaserScan>("Robot0_laser", 10));
        	odom_pubs_.push_back(n_.advertise<nav_msgs::Odometry>("Robot0_odo", 10));
		ground_truth_pubs_.push_back(n_.advertise<nav_msgs::Odometry>("Robot0_truth", 10));		
		cmdvel_subs_.push_back(n_.subscribe<geometry_msgs::Twist>("Robot0_vel", 10, boost::bind(&StageNode::cmdvelReceived, this, r, _1)));
		ROS_INFO("subscribed to Robot0_vel");
		ROS_INFO("RobotID: %i", r);
	}
	else if (r==1)
	{
		laser_pubs_.push_back(n_.advertise<sensor_msgs::LaserScan>("Robot1_laser", 10));
        	odom_pubs_.push_back(n_.advertise<nav_msgs::Odometry>("Robot1_odo", 10));
		ground_truth_pubs_.push_back(n_.advertise<nav_msgs::Odometry>("Robot1_truth", 10));			
		cmdvel_subs_.push_back(n_.subscribe<geometry_msgs::Twist>("Robot1_vel", 10, boost::bind(&StageNode::cmdvelReceived2, this, r, _1)));
		ROS_INFO("subscribed to Robot1_vel");
		ROS_INFO("RobotID: %i", r);
	}
	else if (r==2)
	{
		laser_pubs_.push_back(n_.advertise<sensor_msgs::LaserScan>("Robot2_laser", 10));
        	odom_pubs_.push_back(n_.advertise<nav_msgs::Odometry>("Robot2_odo", 10));
		ground_truth_pubs_.push_back(n_.advertise<nav_msgs::Odometry>("Robot2_truth", 10));			
		cmdvel_subs_.push_back(n_.subscribe<geometry_msgs::Twist>("Robot2_vel", 10, boost::bind(&StageNode::cmdvelReceived3, this, r, _1)));
		ROS_INFO("subscribed to Robot2_vel");
		ROS_INFO("RobotID: %i", r);
	}
	else if (r==3)
	{
		laser_pubs_.push_back(n_.advertise<sensor_msgs::LaserScan>("Robot3_laser", 10));
        	odom_pubs_.push_back(n_.advertise<nav_msgs::Odometry>("Robot3_odo", 10));
		ground_truth_pubs_.push_back(n_.advertise<nav_msgs::Odometry>("Robot3_truth", 10));			
		cmdvel_subs_.push_back(n_.subscribe<geometry_msgs::Twist>("Robot3_vel", 10, boost::bind(&StageNode::cmdvelReceived4, this, r, _1)));
		ROS_INFO("subscribed to Robot3_vel");
		ROS_INFO("RobotID: %i", r);
	}
	else if (r==4)
	{
		laser_pubs_.push_back(n_.advertise<sensor_msgs::LaserScan>("Robot4_laser", 10));
